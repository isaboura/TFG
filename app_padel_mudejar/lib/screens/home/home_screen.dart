                ...
                children: [
                  imagenUrl != null
                      ? CachedNetworkImage(
                          ...
                          fit: BoxFit.cover,
                          placeholder: (context, url) => Container(height: 160, color: Colors.grey.shade100,
                              child: const Center(child: CircularProgressIndicator(color: AppTheme.primary))),
                          errorWidget: (context, url, error) => Container(height: 160, color: Colors.grey.shade100,
                              child: const Icon(Icons.sports_tennis_rounded, size: 60, color: AppTheme.textLight)),
                        )
                      : Container(height: 160, color: Colors.grey.shade100,
                          child: const Center(child: Icon(Icons.sports_tennis_rounded, size: 60, color: AppTheme.textLight))),
                  Positioned(
                    top: 12, left: 12,
                    child: Container(
                  ...
                  if (!activa) Container(height: 160, color: Colors.black.withOpacity(0.4)),
                ],
              ),
            ),
            Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (instalacion['ubicacion'] != null) ...[
                          ...
                          Row(children: [
                            const Icon(Icons.location_on_rounded, size: 12, color: AppTheme.textLight),
                            const SizedBox(width: 3),
                            Text(instalacion['ubicacion'], style: const TextStyle(fontSize: 12, color: AppTheme.textLight)),
                          ]),
                        ],
                        if (promedio != null) ...[
                            ...
                            child: Row(
                              children: [
                                ...List.generate(5, (i) => Icon(
                                  i < promedio.floor() ? Icons.star_rounded
                                      : (i < promedio && promedio % 1 >= 0.5) ? Icons.star_half_rounded
                                ...
                                const Icon(Icons.chevron_right_rounded, size: 14, color: AppTheme.textLight),
                              ],
                            ),
                          ),
                        ] else ...[
                          const SizedBox(height: 8),
                          const Text('Sin reseñas aún', style: TextStyle(fontSize: 12, color: AppTheme.textLight)),
                        ],
                      ],
                    ),
                  ),
...
class _ResenasPistaBottomSheetState extends State<_ResenasPistaBottomSheet> {
  ...
  Future<void> _eliminar(int idResena) async {
    ...
    if (result['success'] == true) {
      messenger.showSnackBar(const SnackBar(content: Text('Reseña eliminada'), backgroundColor: AppTheme.primary));
      _recargar();
    } else {
      messenger.showSnackBar(SnackBar(content: Text(result['message'] ?? 'Error al eliminar'), backgroundColor: AppTheme.danger));
    }
  }
      ...
      builder: (ctx) => _EditarResenaBottomSheet(
        ...
        onActualizada: () {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Reseña actualizada'), backgroundColor: AppTheme.primary));
          _recargar();
        },
      ),
    );
      ...
      child: Column(
        ...
        children: [
          ...
          if (_promedio != null)
            Row(
              children: [
                Text('$_promedio', style: const TextStyle(fontSize: 36, fontWeight: FontWeight.w800, color: AppTheme.accent)),
                const SizedBox(width: 12),
                Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Row(children: List.generate(5, (i) => Icon(
                      i < _promedio!.floor() ? Icons.star_rounded
                          : (i < _promedio! && _promedio! % 1 >= 0.5) ? Icons.star_half_rounded
                          : Icons.star_outline_rounded,
                      color: AppTheme.accent, size: 20,
                    ))),
                    Text('$_total reseñas', style: const TextStyle(fontSize: 12, color: AppTheme.textMedium)),
                  ],
                ),
              ],
            ),
          ...
          Expanded(
            child: _lista.isEmpty
                ? const Center(child: Text('Sin reseñas aún', style: TextStyle(color: AppTheme.textMedium)))
                : ListView.builder(
                        ...
                        child: Row(
                          ...
                          children: [
                            CircleAvatar(
                              ...
                              child: Text(autor.isNotEmpty ? autor[0].toUpperCase() : 'A',
                                ...
                                children: [
                                  Row(
                                    ...
                                    children: [
                                      ...
                                      if (esMia) ...[
                                        GestureDetector(onTap: () => _mostrarEditar(r),
                                            child: const Icon(Icons.edit_rounded, size: 16, color: AppTheme.secondary)),
                                        const SizedBox(width: 8),
                                        GestureDetector(onTap: () => _eliminar(idResena),
                                            child: const Icon(Icons.delete_outline_rounded, size: 16, color: AppTheme.danger)),
                                        const SizedBox(width: 4),
                                      ] else
                                        Text(fechaCorta, style: const TextStyle(fontSize: 11, color: AppTheme.textLight)),
                                    ],
                                  ),
                                  if (esMia) Text(fechaCorta, style: const TextStyle(fontSize: 11, color: AppTheme.textLight)),
                                  const SizedBox(height: 4),
                                  Row(children: List.generate(5, (j) => Icon(
                                    j < puntuacion ? Icons.star_rounded : Icons.star_outline_rounded,
                                    color: AppTheme.accent, size: 14,
                                  ))),
                                  if (comentario != null && comentario.isNotEmpty) ...[
                                    const SizedBox(height: 4),
                                    Text(comentario, style: const TextStyle(fontSize: 13, color: AppTheme.textMedium)),
                                  ],
                                ],
                              ),
                            ),
...
class _EditarResenaBottomSheet extends StatefulWidget {
  ...
  const _EditarResenaBottomSheet({required this.resena, required this.dniSocio, required this.onActualizada});
...
class _EditarResenaBottomSheetState extends State<_EditarResenaBottomSheet> {
  ...
  void initState() {
    ...
    _comentarioCtrl = TextEditingController(text: widget.resena['comentario'] as String? ?? '');
  }
      ...
      child: Column(
        ...
        children: [
          Center(child: Container(width: 40, height: 4,
              decoration: BoxDecoration(color: Colors.grey.shade300, borderRadius: BorderRadius.circular(2)))),
          const SizedBox(height: 20),
          const Text('Editar reseña', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: AppTheme.textDark)),
          const SizedBox(height: 20),
          const Text('Puntuación', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: AppTheme.textDark)),
          ...
          Row(children: List.generate(5, (i) {
            ...
            return GestureDetector(
              ...
              child: Padding(padding: const EdgeInsets.only(right: 8),
                  child: Icon(estrella <= _puntuacion ? Icons.star_rounded : Icons.star_outline_rounded,
                      color: AppTheme.accent, size: 36)),
            );
          })),
          const SizedBox(height: 20),
          const Text('Comentario', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: AppTheme.textDark)),
          const SizedBox(height: 10),
          TextField(
